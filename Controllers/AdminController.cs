    ...
    public class AdminController : Controller
    {
        ...
        public ActionResult Dashboard()
        {
            ...
            try
            {
                ...
                int todayNewUsers = db.Users.Count(u => u.RegisterDate.Date == today);
                ViewBag.TodayNewUsers = todayNewUsers;

                // 獲取本週新增會員數
                DateTime weekStart = DateTime.Today.AddDays(-(int)DateTime.Today.DayOfWeek);
                int weekNewUsers = db.Users.Count(u => u.RegisterDate.Date >= weekStart && u.RegisterDate.Date <= today);
                ViewBag.WeekNewUsers = weekNewUsers;

                // 獲取總收藏次數 TOP5 文獻
                var topCollectedPapers = (from p in db.Paper
                                          join c in db.Collection on p.PaperId equals c.PaperId
                                          group c by new { p.PaperId, p.Title, p.Authors, p.Year } into g
                                          select new
                                          {
                ...
                ViewBag.TopCollectedPapers = topCollectedPapers;

                // 獲取最常被瀏覽的文獻（依點擊數）
                var topViewedPapers = db.Paper
                                      .OrderByDescending(p => p.ClickCount)
                                      .Take(5)
                                      .ToList();

                ViewBag.TopViewedPapers = topViewedPapers;

                // 獲取評論數量最多的文獻
                var topCommentedPapers = (from p in db.Paper
                                          join c in db.Comment on p.PaperId equals c.PaperId
                                          group c by new { p.PaperId, p.Title, p.Authors } into g
                                          select new
                                          {
                                              PaperId = g.Key.PaperId,
                                              Title = g.Key.Title,
                ...
                ViewBag.TopCommentedPapers = topCommentedPapers;

                return View();
            }
            catch (Exception ex)
            {
                ViewBag.ErrorMessage = "獲取統計數據時發生錯誤: " + ex.Message;
                return View();
            }
        }
        ...
        // 新增文獻
        [HttpPost]
        public ActionResult Paper_Add(Paper model, HttpPostedFileBase pdfFile, HttpPostedFileBase previewImage, int[] selectedCategories)
        {
            var checkResult = CheckAdminAccess();
            if (checkResult != null)
                return checkResult;
